...
| `bayes_diary.py` | 贝叶斯日记（页面在 `/bayes-diary`：给日常预测下注 -> 记证据实时更新后验 -> 结算后画校准曲线，历史记录按天回看与导入导出备份，纯浏览器计算） |
...
2. **随手记证据**：出现新信息时只回答一个问题 —— 这条证据在「会发生」和「不会发生」下各有多大可能出现？两个滑杆换出一个似然比；
3. **看后验动**：概率条随滑杆实时滑动，直觉和数字差多少一眼就能看到；
4. **隔天结算**：结果出来后点「发生了 / 没发生」，日积月累画出校准曲线（你嘴上说 80% 的事，实际发生了多少次），并给出 Brier 分数；
5. **回看历史**：所有预测按天分组列在「历史记录」里，可按「全部 / 待结算 / 已结算」筛选；还能导出 JSON 备份、导入或清空；
6. **彩蛋实验**：同一个信念，先喂证据 A 再喂 B、和先 B 后 A，中间过程不同但终点必然完全一致。

说明：

- 全部概率计算（赔率 × 似然比）与数据存储都在浏览器本地完成（localStorage），后端只负责返回页面，无数据库、无密钥；
- 结算时记录的是你当时的概率，所以中途记过证据的预测会按更新后的信念计入校准曲线；
- 数据只保存在当前浏览器里，换设备或清理缓存会丢失，建议定期用「导出备份」保存一份 JSON。
## 打包成 EXE（可选）
