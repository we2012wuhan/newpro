...
| `bayes_diary.py` | 贝叶斯日记（页面在 `/bayes-diary`：给日常预测下注 -> 记证据更新后验 -> 结算后出校准报告，计算与存储全在 Python + SQLite） |
...
打开 `http://127.0.0.1:8000/bayes-diary`：

1. **下注**：写一条当天就能验证的预测（如“明天 9 点前外卖会准时送到”），再用滑杆给出你心里的概率；
2. **随手记证据**：出现新信息时只回答一个问题 —— 这条线索在「会发生」和「不会发生」下各有多大可能出现？五档快选（或自己填两个概率）换出一个似然比；
3. **看后验动**：每记一条，概率条就动一次，并标出信念从多少走到多少、走了几个百分点；
4. **隔天结算**：结果出来后点「发生了 / 没发生」，日积月累画出校准曲线（你嘴上说 80% 的事，实际发生了多少次），并给出 Brier 分数；
5. **彩蛋实验**：同一个信念，先喂证据 A 再喂 B、和先 B 后 A，中间过程不同但终点必然完全一致。

说明：

- 全部计算在后端 Python 完成：`bayes_math.py` 负责似然比与后验更新，`bayes_diary.py` 负责校准评估（Brier / 对数损失 / ECE / AUC / Murphy 分解）并把这些数字翻译成「认知提醒」；前端只负责收集输入和画图；
- 数据存在 SQLite（`data/bayes.db`），换浏览器、换设备都还在，不再依赖 localStorage；
- 三条规则写死在后端，为的是保住数据可信度：先验保存后不能改（PATCH 带 `prior` 直接 400）、结算后不能再加证据（409）、结果只能记一次（409）；
- 后验永远由 Python 现算，不存结果值，所以删掉中间任意一条证据，时间线都会重算而不是留着脏数字。
## 打包成 EXE（可选）
